...
- 缺陷训练偏好：对有学习价值的功能问题，优先告知现象、预期行为和最小复现入口，由用户独立查找并首次修复；Agent按需逐级给线索，之后Review和硬验证。不要默认一次揭示全部根因、交付修复答案或顺手修完；功能缺陷的完整诊断可折叠供复现后核对。耦合、聚合、职责划分、依赖方向等结构问题直接指出并解释，给出具体调整建议，不作为自主复现训练题；已授权的结构整理、机械重命名、环境与非核心配置可直接完成。
...
- 每个实验需要留下能比较“原模型 / 实际证据 / 模型改变”的收尾证据，但不强制用户按三行模板回复。用户自发表达、连续Diff中的修正、测试预言与硬证据已经足以显示模型变化时，由Agent据实汇总；只有无法判断关键模型是否改变时才追问一个最小问题。不得把自解释变成填表或复述Agent答案。
